/**
 * T4 — gdrive MCP: recorded-fixture functional tests.
 *
 * Requires owner to run: npm run record:fixtures -- gdrive
 * Fixtures land at: test/fixtures/recordings/gdrive/
 */
...
const FIXTURES_DIR = resolve(process.cwd(), 'test/fixtures/recordings/gdrive');
...
describe('gdrive MCP — recorded fixtures', () => {
  const fixtures = loadFixtures();

  if (fixtures.length === 0) {
    it.skip('requires recording — run: npm run record:fixtures -- gdrive', () => {
  ...
  describe('fixture shape validation', () => {
    for (const fixture of fixtures) {
      it(`${fixture.tool} — fixture is well-formed`, () => {
        expect(fixture.server).toBe('gdrive');
