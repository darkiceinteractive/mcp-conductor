/**
 * T4 — memory MCP: recorded-fixture functional tests.
 *
 * Requires owner to run: npm run record:fixtures -- memory
 * Fixtures land at: test/fixtures/recordings/memory/
 */
...
const FIXTURES_DIR = resolve(process.cwd(), 'test/fixtures/recordings/memory');
...
describe('memory MCP — recorded fixtures', () => {
  const fixtures = loadFixtures();

  if (fixtures.length === 0) {
    it.skip('requires recording — run: npm run record:fixtures -- memory', () => {
  ...
  describe('fixture shape validation', () => {
    for (const fixture of fixtures) {
      it(`${fixture.tool} — fixture is well-formed`, () => {
        expect(fixture.server).toBe('memory');
