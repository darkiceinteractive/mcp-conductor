/**
 * T4 — slack MCP: recorded-fixture functional tests.
 *
 * Requires owner to run: npm run record:fixtures -- slack
 * Fixtures land at: test/fixtures/recordings/slack/
 */
...
const FIXTURES_DIR = resolve(process.cwd(), 'test/fixtures/recordings/slack');
...
describe('slack MCP — recorded fixtures', () => {
  const fixtures = loadFixtures();

  if (fixtures.length === 0) {
    it.skip('requires recording — run: npm run record:fixtures -- slack', () => {
  ...
  describe('fixture shape validation', () => {
    for (const fixture of fixtures) {
      it(`${fixture.tool} — fixture is well-formed`, () => {
        expect(fixture.server).toBe('slack');
