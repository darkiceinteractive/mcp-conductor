/**
 * T4 — brave-search MCP: recorded-fixture functional tests.
 *
 * Requires owner to run: npm run record:fixtures -- brave-search
 * Fixtures land at: test/fixtures/recordings/brave-search/
 */
...
const FIXTURES_DIR = resolve(process.cwd(), 'test/fixtures/recordings/brave-search');
...
describe('brave-search MCP — recorded fixtures', () => {
  const fixtures = loadFixtures();

  if (fixtures.length === 0) {
    it.skip('requires recording — run: npm run record:fixtures -- brave-search', () => {
  ...
  describe('fixture shape validation', () => {
    for (const fixture of fixtures) {
      it(`${fixture.tool} — fixture is well-formed`, () => {
        expect(fixture.server).toBe('brave-search');
