/**
 * T4 — notion MCP: recorded-fixture functional tests.
 *
 * Requires owner to run: npm run record:fixtures -- notion
 * Fixtures land at: test/fixtures/recordings/notion/
 */
...
const FIXTURES_DIR = resolve(process.cwd(), 'test/fixtures/recordings/notion');
...
describe('notion MCP — recorded fixtures', () => {
  const fixtures = loadFixtures();

  if (fixtures.length === 0) {
    it.skip('requires recording — run: npm run record:fixtures -- notion', () => {
  ...
  describe('fixture shape validation', () => {
    for (const fixture of fixtures) {
      it(`${fixture.tool} — fixture is well-formed`, () => {
        expect(fixture.server).toBe('notion');
