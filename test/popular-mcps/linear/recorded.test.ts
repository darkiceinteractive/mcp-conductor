/**
 * T4 — linear MCP: recorded-fixture functional tests.
 *
 * Requires owner to run: npm run record:fixtures -- linear
 * Fixtures land at: test/fixtures/recordings/linear/
 */
...
const FIXTURES_DIR = resolve(process.cwd(), 'test/fixtures/recordings/linear');
...
describe('linear MCP — recorded fixtures', () => {
  const fixtures = loadFixtures();

  if (fixtures.length === 0) {
    it.skip('requires recording — run: npm run record:fixtures -- linear', () => {
  ...
  describe('fixture shape validation', () => {
    for (const fixture of fixtures) {
      it(`${fixture.tool} — fixture is well-formed`, () => {
        expect(fixture.server).toBe('linear');
